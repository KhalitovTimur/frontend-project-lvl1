...
const mindGame = async () => {
  ...
  for (let i = 0; i < 3; i += 1) {
    const randomNumberFirst = randomInteger(minNumberForRandomGeneration, maxNumberForRandomGeneration);
    const randomNumberSecond = randomInteger(minNumberForRandomGeneration, maxNumberForRandomGeneration);
    const answer = await promptly.prompt(`Question: ${randomNumberSecond} ${randomNumberFirst}: `);
